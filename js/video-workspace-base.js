...
function getAutomatableControls(definition) {
  return Array.isArray(definition?.controls) ? definition.controls : [];
}
...
function sanitizeAutomationEndpoint(control, value, fallback) {
  ...
  const fallbackValue = Number.isFinite(Number(fallback)) ? Number(fallback) : control.min;
  return clampControlValue(control, Number.isFinite(Number(value)) ? Number(value) : fallbackValue);
}

function cloneAutomationConfig(control, config, fallbackValue) {
  const safeValue = sanitizeAutomationEndpoint(control, fallbackValue, control.min);
  return {
...
function cloneAutomations(definition, params = {}, automations = {}) {
  return getAutomatableControls(definition).reduce((nextAutomations, control) => {
    const fallbackValue = params[control.key] ?? control.min;
    nextAutomations[control.key] = cloneAutomationConfig(control, automations?.[control.key], fallbackValue);
...
function formatNumericControlValue(control, value) {
  return Number(value).toFixed(getControlDigits(control));
}

function formatAutomationControlValue(control, value) {
  if (control?.kind === "toggle") {
...
function renderAutomationMarkup(control, config) {
  const digits = getControlDigits(control);
  const startValue = formatAutomationControlValue(control, config.start);
  const endValue = formatAutomationControlValue(control, config.end);
  return `
    <div class="layer-automation-row${config.enabled ? " is-enabled" : ""}" data-automation-key="${control.key}">
      <div class="layer-automation-topline">
        <span class="layer-automation-name">${control.label}</span>
        <label class="layer-automation-enable">
          <input data-automation-toggle="${control.key}" type="checkbox" ${config.enabled ? "checked" : ""} />
          <span>Enable</span>
        </label>
      </div>
      <div class="layer-automation-grid">
        <label>
          <span>Start</span>
          ${control.kind === "toggle" ? `
            <select data-automation-start="${control.key}">
              <option value="true" ${config.start ? "selected" : ""}>On</option>
              <option value="false" ${config.start ? "" : "selected"}>Off</option>
            </select>
          ` : `
            <input
              data-automation-start="${control.key}"
              data-automation-digits="${digits}"
              type="number"
              min="${control.min}"
              max="${control.max}"
              step="${control.step}"
        ...
        <label>
          <span>End</span>
          ${control.kind === "toggle" ? `
            <select data-automation-end="${control.key}">
              <option value="true" ${config.end ? "selected" : ""}>On</option>
              <option value="false" ${config.end ? "" : "selected"}>Off</option>
            </select>
          ` : `
            <input
              data-automation-end="${control.key}"
              data-automation-digits="${digits}"
              type="number"
              min="${control.min}"
              max="${control.max}"
        ...
        <label>
          <span>Curve</span>
          <select data-automation-curve="${control.key}">
            ${AUTOMATION_CURVES.map((option) => `
              <option value="${option.value}" ${option.value === config.curve ? "selected" : ""}>${option.label}</option>
            `).join("")}
          </select>
        </label>
      </div>
...
export function createVideoWorkspace({
  ...
  function evaluateAutomation(control, baseValue, config, progress) {
    ...
    if (control.kind === "toggle") {
      return t < 0.5 ? Boolean(start) : Boolean(end);
    }
    return clamp(start + (end - start) * t, control.min, control.max);
  }

  function evaluateLayerParams(layer, definition) {
  ...
  function withEvaluatedLayerParams(layer, callback) {
    const definition = effectMap.get(layer?.type);
    const originalParams = layer?.params;
    const evaluatedParams = layer && originalParams ? evaluateLayerParams(layer, definition) : originalParams;

    if (!layer || !originalParams || evaluatedParams === originalParams) {
      return callback();
    }

    layer.params = evaluatedParams;
    try {
      return callback();
    } finally {
      layer.params = originalParams;
    }
  }
  ...
  function buildInlineControls(layer) {
    ...
    const controlsMarkup = definition.controls
      .map((control) => renderControlMarkup(control, layer.params[control.key]))
      .join("");
    const automatableControls = getAutomatableControls(definition);
    if (!automatableControls.length) {
      return controlsMarkup;
    }

    const automationMarkup = automatableControls
      .map((control) => renderAutomationMarkup(
        control,
        layer.automations?.[control.key] || cloneAutomationConfig(control, null, layer.params[control.key]),
      ))
      .join("");

    return `
      ${controlsMarkup}
      <div class="layer-automation-shell">
        <div class="layer-automation-head">
          <span>Automation</span>
          <button class="mini-button" data-action="toggle-automation" type="button">${layer.automationOpen ? "▾" : "▸"}</button>
        </div>
        ${layer.automationOpen ? `<div class="layer-automation-panel" data-automation-layer="${layer.id}">${automationMarkup}</div>` : ""}
      </div>
    `;
  }
  ...
  function syncAutomationControlViews(layer, key, config, control, activeInput = null) {
    if (!layer || !config || !control) {
      return;
    }

    const layerSelector = `[data-automation-layer="${layer.id}"]`;
    const startValue = formatAutomationControlValue(control, config.start);
    const endValue = formatAutomationControlValue(control, config.end);

    document.querySelectorAll(`${layerSelector} [data-automation-key="${key}"]`).forEach((row) => {
      row.classList.toggle("is-enabled", Boolean(config.enabled));
    });

    document.querySelectorAll(`${layerSelector} [data-automation-toggle="${key}"]`).forEach((toggle) => {
      if (toggle !== activeInput) {
        toggle.checked = Boolean(config.enabled);
      }
    });

    document.querySelectorAll(`${layerSelector} [data-automation-start="${key}"]`).forEach((input) => {
      if (input !== activeInput) {
        input.value = startValue;
      }
    });

    document.querySelectorAll(`${layerSelector} [data-automation-end="${key}"]`).forEach((input) => {
      if (input !== activeInput) {
        input.value = endValue;
      }
    });

    document.querySelectorAll(`${layerSelector} [data-automation-curve="${key}"]`).forEach((select) => {
      if (select !== activeInput) {
        select.value = config.curve;
      }
    });
  }

  function syncDormantAutomationWithBaseValue(layer, key, control, previousValue, nextValue) {
    if (!layer || !control) {
      return;
    }

    const config = layer.automations?.[key];
    if (!config || config.enabled) {
      return;
    }

    if (control.kind === "toggle") {
      if (config.start === previousValue && config.end === previousValue) {
    ...
    if (
      ...
      && Math.abs(config.start - previousNumber) <= tolerance
      && Math.abs(config.end - previousNumber) <= tolerance
    ) {
      config.start = nextValue;
      config.end = nextValue;
      syncAutomationControlViews(layer, key, config, control);
  ...
  function bindInlineControls(container, layer) {
    ...
    const controlMap = new Map(definition.controls.map((control) => [control.key, control]));
    ...
    container.querySelectorAll("[data-inline-slider]").forEach((slider) => {
      ...
      slider.addEventListener("input", () => {
        ...
        const previousValue = layer.params[key];
    ...
    container.querySelectorAll("[data-automation-toggle]").forEach((toggle) => {
      toggle.addEventListener("click", (event) => event.stopPropagation());
      toggle.addEventListener("change", () => {
        const key = toggle.dataset.automationToggle;
        const control = controlMap.get(key);
        if (!control) {
          return;
        }
    ...
    container.querySelectorAll("[data-automation-start], [data-automation-end]").forEach((input) => {
      input.addEventListener("click", (event) => event.stopPropagation());
      input.addEventListener("change", () => {
        const key = input.dataset.automationStart || input.dataset.automationEnd;
        const control = controlMap.get(key);
        if (!control) {
          return;
        }
    ...
    container.querySelectorAll("[data-automation-curve]").forEach((select) => {
      select.addEventListener("click", (event) => event.stopPropagation());
      select.addEventListener("change", () => {
        const key = select.dataset.automationCurve;
        const control = controlMap.get(key);
        if (!control) {
          return;
        }
  ...
  function randomizeLayerParams(layer, definition) {
    ...
    layer.automations = cloneAutomations(definition, layer.params, layer.automations);
  }
  ...
  function applyPreset(preset) {
    ...
    layerEntries.forEach((entry) => {
      ...
      layer.automations = cloneAutomations(
        definition,
        layer.params,
